...
class Table(object):
    ...
    def __init__(self, client, database, table):
        try:
            ...
            raise ValueError('faild to create pytd.table.Table instance for `{}.{}`: {}'.format(database, table, e))
    ...
    def create(self, column_names=[], column_types=[]):
        ...
        if len(column_names) > 0:
            schema = ', '.join(map(lambda t: '{} {}'.format(t[0], t[1]), zip(column_names, column_types)))
            q_create = 'CREATE TABLE {}.{} ({})'.format(self.database, self.table, schema)
            self.client.query(q_create)
        else:
    ...
    def bulk_import(self, dataframe, if_exists='error'):
        ...
        bulk_import = self.client.api_client.create_bulk_import(session_name, self.database, self.table)
        try:
        ...
        if 0 < bulk_import.error_records:
            logger.warning('detected {} error records.'.format(bulk_import.error_records))
    ...
    def spark_import(self, dataframe, if_exists='error', td_spark_path=None, download_if_missing=True):
        """Write a given DataFrame to a Treasure Data table.
        ...
        if self.td_spark is None:
            ...
            if 'eu01' in self.client.endpoint:
                site = 'eu01'
            self.td_spark = self._fetch_td_spark(self.client.apikey, site, td_spark_path, download_if_missing)
        ...
        dataframe = dataframe.rename(lambda c: re.sub(r'[^a-zA-Z0-9]', ' ', str(c)).lower().replace(' ', '_'), axis='columns')

        sdataframe = self.td_spark.createDataFrame(dataframe)
        try:
            sdataframe.write.mode(if_exists).format('com.treasuredata.spark').option('table', destination).save()
            ...
            raise RuntimeError('failed to load table via td-spark: {}'.format(e.java_exception))
    ...
    def _fetch_td_spark(self, apikey, site, td_spark_path, download_if_missing):
        ...
        pyspark-shell
        """.format(td_spark_path, apikey, site)
    ...
    def _download_td_spark(self, destination):
        ...
        try:
            ...
            raise RuntimeError('failed to access to the download URL: {}'.format(download_url))
